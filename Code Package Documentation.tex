...
\begin{figure}[h]
\caption{Function List (excluding third-party functions}
\begin{center}
...
\texttt{File\_Domain}&Aux&Sets file to save figures in\\
...
\texttt{Spectral\_Ratios}&Aux&Calculate ratio of variability or summary statistics\\
...
\texttt{Maps\_ratio}&Mapping&Map changes in variability\\ 
\texttt{Maps\_diagnostics}&Mapping&Map diagnostics/summary statistics of a variable\\
\texttt{MapLimits}&Aux&Set colorbar limits across models for consistent plots\\
\texttt{data\_colormap}&Aux&Assign colors to array elements\\
...
\texttt{clip\_var\_min}&Aux&Get indices of pixels above a certain value\\
\texttt{clip\_var\_max}&Aux&Get indices of pixels below a certain value\\
\texttt{subset\_find}&Aux&Find indices of one array within another\\
\texttt{subplot\_pos}&Aux&A collection of figure-friendly subplot positions\\
\texttt{loc\_descriptor}&Aux&Get human-readable political description of a lat/lon pair\\
\texttt{quote\_me}&Aux&Just spits out a positive quote about science\\
\end{tabular}
\end{center}
\end{figure}
...
Any data that follows the CMIP5 naming convention and the required NetCDF file format should be processable without any modification of \texttt{Saves.m}, with built-in options for setting the variable and attribute names to look for in the \texttt{.nc} file when they are different than the CMIP5 format included in the program. See the \texttt{Saves.m} header (by calling \texttt{help Saves}) for full details. 
...
Though the reader is encouraged to refer to the header of each function for an exhaustive list of function flags and program run modifications (by typing \texttt{help [function name]} into the MATLAB console), a few flags are general across the code package and are worth enumerating here:

\begin{description}
\item [`testing'] adds the suffix \texttt{\_TEST} to the output filename(s)
\item [`experiments'] sets the experiment runs processed, in cell array format (e.g.\ \texttt{\{rcp85,piControl\}}, must be written as in filenames; different display names, for example RCP8.5 instead of rcp85, are supported through changing the \textbf{expArray\_disp} array in \texttt{various\_defaults.m})). Some code, such as \texttt{Maps\_ratio.m} or \texttt{SpectralRatios.m}, requires multiple experiment inputs in a specific order, please see function headers for exact requirements. 
\end{description}
...
Intra- and inter-seasonal variability can be calculated through \texttt{Variability\_seasons.m}. As with \texttt{Variability.m}, the function \texttt{Variability\_seasons.m} takes in a \texttt{\_DATA.mat} file and outputs two files containing the local means and a struct with variability and summary statistics data, though now with the addition of a seasonal suffix: \texttt{\_LocalMeans\_[seas].mat} and \texttt{\_StdDevs\_[seas].mat}, respectively. The suffix \texttt{[seas]} is set by default in \texttt{various\_defaults.mat} and is generally a shorthand of the included months (i.e. \texttt{DJF} for Dec-Jan-Feb). By default, \texttt{DJF} and \texttt{JJA} are processed; for other seasons, please see the header of the function. 

The frequency bounds for intra-seasonal variability calculations are taken as before from \texttt{various\_defaults.mat} (the variable \textbf{freq\_bands\_setup\_seasons}), with customizable bounds set in the function call (see the header). Interseasonal variability (\texttt{$>$ 1 year}) is defined as the standard deviation of seasonal means across years in the data set. 

By default, the function processes 30 seasons. If the start and end dates of a season are in different calendar years, the function will attempt to take data from 31 (or \texttt{[nyears] + 1} for custom year lengths) years of raw data to keep the total number of seasons contained in the analysis consistent. Warnings will be thrown if this is not possible. 

\subsection{Calculating Variability Ratios}
\label{func:SpectralRatios.m}
For a slight simplification in manual examination of variability change data, the code package allows for the creation of extra files that store the ratio of frequency-separated variability between two experiments (these files are also necessary to run \texttt{Maps\_ratio.m}). This process occurs through the program \texttt{SpectralRatios.m}. The function is called as normal, with an important input to remember:
\begin{equation}
...
\subsection{Mapping ratios of variability changes}
\label{func:Maps_ratio.m}
The code package includes some basic mapping tools. The most robust, and most used in the ``Atlas of Variability'' Project is \texttt{Maps\_ratio.m}, which produces a map / up to 6 subplotted maps of the change in variability between two experiments of a variable - frequency - model combination, and can hash out pixels without a meaningful change in variability, as calculated in \texttt{Variability\_CI.m}. It requires a previously calculated \texttt{\_SpectralRatios.mat} file for the experiments to be compared, and, if a marking of pixels without meaningful changes is desired, a previously calculated \texttt{\_StdDevsCI.mat} file.

Beyond the general functional form (as always, check the function header through \texttt{help Maps\_ratio} for all flags and details), I would like to point out two specific function flags:
\begin{description}
...
\subsection{Mapping some summary statistics}
\label{func:maps_diagnostics}
In addition to \texttt{Maps\_ratio.m}, this package also includes the mapping function \texttt{Maps\_diagnostics.m}, which creates a set of 4-panel maps showing 1) mean values for \textit{exp$_1$}, 2) \textit{exp$_1$} standard deviations, 3) difference in mean values \textit{exp$_2$} - \textit{exp$_1$}, and 4) ratio of standard deviations $\frac{exp_2}{exp_1} $for a variable - model combination. By default, \textit{exp$_1$} and \textit{exp$_2$} are \texttt{piControl} and \texttt{rcp85}, respectively. See the function header for more detail. 

\texttt{Maps\_diagnostics.m} requires color limits across models that it takes from running \texttt{MapLimits.m}. \texttt{MapLimits.m} creates a file with intermodel statistics that allow for a consistent intermodel colormap creation (these are \textit{not} rigorous summary statistics). See the function header for more detail.

\section{Graphing Results} %% GRAPHING RESULTS
\label{func:Plots_Log_Panel_prod.m}
Explicit graphing functions (beyond the mapping functions detailed above) are not fully supported in this code package (a beta scatterplotting function, \texttt{panel\_plots.m} is included; see its header for a description and use at your own risk); however, certain auxiliary tools are included to make graphing easier. 
...
\subsection{Getting subsets of arrays}
\texttt{subset\_find.m} allows for finding the indices of subsets of arrays within larger arrays. The basic call is as follows: 
\begin{equation}
\textbf{[x\_idxs,y\_idxs]} = \texttt{subset\_find(}\textbf{x},\textbf{y},\textbf{x'},\textbf{y'}\texttt{)}
\end{equation}
where the matrix (\textbf{x',y'}) is a subset of the matrix (\textbf{x,y}), and \textbf{x\_idxs,y\_idxs} are the indices of each point in (\textbf{x',y'})'s position in (\textbf{x,y}). More specifically, in terms of climate data, this procedure allows finding a geographic subset inside a lat/lon grid, through the (identical) call 
\begin{equation}
\textbf{[lat\_idxs,lon\_idxs]} = \texttt{subset\_find(}\textbf{lat},\textbf{lon},\textbf{lat'},\textbf{lon'}\texttt{)}
\end{equation}

\subsection{Getting a human-readable location description}
\texttt{loc\_descriptor.m} uses the Google Maps database to return a human-readable description of a lat/lon location. By default,
\begin{equation}
\textbf{loc\_desc} = \texttt{loc\_descriptor(lat,lon)}
\end{equation}
returns the most complete form of the location's address stored in the Google Maps database. For more comp[ex reverse geocoding (such as just getting the political division using the \texttt{`result\_type', `political'} flag) requires your own API key (see function header), which can be inputted using the \texttt{`api\_key'} flag. 


\section{Third Party/Community Functions} %% ADDITIONAL FUNCTIONS / FILES
A few functions (listed in Figure \ref{table:comm_funcs}) are taken from the MATLAB community. 

\begin{figure}[h]
\caption{Third-Party Function List}\label{table:comm_funcs}
\begin{center}
\begin{tabular}{l p{0.8in} | p{2.7in} | p{1in} }
Function Name&Type&Description&Author\\ \hline
\texttt{ds2nfu}&Plotting&Converts from data units to normalized figure units for placing objects onto plots&Michelle Hirsch\\
\texttt{piecelin}&Calculation&Used for piecewise interpolation of colormaps in \texttt{data\_colormaps.m}&\\
\texttt{structfind}&Aux&Gets index of a struct based on matching a specific field&Dirk-Jan Kroon\\
\texttt{rgb}&Plotting&Returns RGB triplet for CSS3 colors&Kristjan Jonasson\\
\texttt{wraptext}&Aux&Wraps output text in console&Chad Greene\\ 
\end{tabular}
\end{center}
\end{figure}
...
\item run \texttt{Maps\_ratio(5,\{`CSIRO-Mk3-6-0'\}, `experiments',\{`rcp85',`piControl'\},}\\ \texttt{`show\_freqs',\{`HF',`MF',`mean',`LF'\})} and \texttt{Maps\_ratio(5,\{`CSIRO-Mk3-6-0'\}, \\`experiments',\{`rcp85',`historical'\}, `show\_freqs',\{`HF',`MF',`mean',`LF'\})} (Section \ref{func:Maps_ratio.m}) to map the $<$ 5 day variability, 5 - 30 day variability, mean, and 30 - 365 day variability changes in daily precipitation between RCP8.5 and PI control and RCP8.5 and historical runs
\item run \texttt{Maps\_diagnostics(5,\{`CSIRO-Mk3-6-0'\}, `experiments',\{`piControl',`rcp85'\})} (Section \ref{func:maps_diagnostics}) to map the pre-industrial mean and standard deviation, the mean change RCP8.5 - PI control and the ratio of standard deviations RCP8.5 / PI control for daily precipitation
\item run \texttt{panel\_plots(5, \{`CSIRO-Mk3-6-0'\}, `experiments', \{`rcp85',`piControl'\}, `show\_freqs', \{`LF',`XF'\}, `clip\_lat',30,55, `color\_by',`data', \{`piControl'\}, \{`mean'\}, `log10')} (Section \ref{func:Plots_Log_Panel_prod.m})\footnote{in beta} to create a scatterplot of variability changes vs.\ mean changes for 30 - 365 day and $>$ 1 year variability (in 2 subplots) in the mid-latitudes ($\abs{30^\circ-55^\circ}$), colored by the base-10 log of pre-industrial mean precipitation
\end{enumerate}
...
\section{Mathematical background and derivations}
\label{app:math}
...
\section{A Few Frequently Used Terms}

\begin{description}
\item [modelArray] a cell array of models (explicitly the 3rd section of each filename, and also the subdirectory where each file is found) used to select files to input into functions
\item [expArray] a cell array of experiments (explicitly the 4th section of each filename) used to select files to input into functions
\item [run] a shorthand name for the computational run that created a certain file (explicitly the 5th section of each filename)
\item [frequency band] a segment in frequency space bounded by two frequencies for which variability is calculated
\item [CMIP5] "Coupled Model Intercomparison Project Phase 5" - a model intercomparison project whose file structure and naming conventions are assumed throughout this code package
